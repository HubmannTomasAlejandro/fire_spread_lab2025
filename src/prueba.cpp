...
// Ángulos pre‑definidos
extern const float ANGLES[8];
...
// SoA: Structure‑of‑Arrays para los vecinos
struct NeighborsSoA {
    alignas(32) float elevation[8];  //probar short
    VegetationType vegetation[8];
    float fwi[8];
    float aspect[8];
};

// “Quemante” (solo los campos usados)
struct Burning {
    float elevation;
    float wind_direction;
};

// Función de conversión de arreglo AoS → SoA
inline void pack_neighbors(const Cell neighbors[N_NEIGH],
                           NeighborsSoA& nb)
{
    for (size_t i = 0; i < N_NEIGH; ++i) {
        nb.elevation[i]   = neighbors[i].elevation;
        nb.vegetation[i]  = neighbors[i].vegetation_type;
        nb.fwi[i]         = neighbors[i].fwi;
        nb.aspect[i]      = neighbors[i].aspect;
    }
}
...
void spread_probability(
    ...
    for (size_t i = 0; i < N_NEIGH; ++i) {
        // Términos básicos
        float diff        = nb.elevation[i] - burning.elevation;
        float slope_term  = sinf(atanf(diff / distance));
        float wind_term   = cosf(ANGLES[i] - burning.wind_direction);
        float elev_term   = (nb.elevation[i] - elevation_mean) / elevation_sd;

        // Construcción del linpred
        float linpred = params.independent_pred;
        switch (nb.vegetation[i]) {
            ...
            default: /* NONE → nada */                        break;
        }
        linpred += params.fwi_pred    * nb.fwi[i]
                + params.aspect_pred * nb.aspect[i]
                + params.wind_pred   * wind_term
                + params.elevation_pred * elev_term
                + params.slope_pred  * slope_term;

        // Cálculo final de la probabilidad
        bool can_burn = (nb.vegetation[i] != NONE) && burnable_cell[i];
        probs[i] = can_burn
